...
public class GeoTIFFVerticalGridShift implements VerticalGridShift {
    ...
    private static final int IN_MEMORY_READ_SIZE_THRESHOLD =
            Integer.getInteger(
                    IN_MEMORY_READ_SIZE_THRESHOLD_KEY, DEFAULT_IN_MEMORY_READ_SIZE_THRESHOLD);
    ...
    RenderedImage gridImage = null;

    /** Whether to use JAI or an Immediate read to load the grid */
    ...
    public GeoTIFFVerticalGridShift(File file, int crsCode) throws IOException {
        ...
        int numBytes = DataBuffer.getDataTypeSize(dataType) / 8;
        if (height * width * numBytes > IN_MEMORY_READ_SIZE_THRESHOLD) {
            useJAI = true;
        }
        ParameterValue<Boolean> useJAIParameter = GeoTiffFormat.USE_JAI_IMAGEREAD.createValue();
        useJAIParameter.setValue(useJAI);

        GridCoverage2D coverage = reader.read(new GeneralParameterValue[] {useJAIParameter});
    ...
    private double readDouble(int gridI, int gridJ) {
        // This type of access will go through cached tiles
        RandomIter iter =
                RandomIterFactory.create(gridImage, new Rectangle(gridI, gridJ, 1, 1), true, true);
        double val = iter.getSampleDouble(gridI, gridJ, 0);
    ...
    private float readFloat(int gridI, int gridJ) {
        ...
        RandomIter iter =
                RandomIterFactory.create(gridImage, new Rectangle(gridI, gridJ, 1, 1), true, true);
        float val = iter.getSampleFloat(gridI, gridJ, 0);
        if (Math.abs(val - noData) < DELTA) {
            val = Float.NaN;
        }
        iter.done();
