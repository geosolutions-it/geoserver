...
public class CatalogBackupTasklet extends AbstractCatalogBackupRestoreTasklet {
    ...
    private void doRestore(JobExecution jobExecution, final GeoServer geoserver,
            final GeoServerDataDirectory dd) throws IOException, UnexpectedJobExecutionException {
        ...
        GeoServerInfo newGeoServerInfo = (GeoServerInfo) doRead(sourceRestoreFolder, "global.xml");
        SettingsInfo newSettings = (SettingsInfo) doRead(sourceRestoreFolder, "settings.xml");
        LoggingInfo newLoggingInfo = (LoggingInfo) doRead(sourceRestoreFolder, "logging.xml");

        // Save Workspace specific settings
        Resource sourceWorkspacesFolder = BackupUtils.dir(sourceRestoreFolder, "workspaces");
        ...
        // TODO: Finish reading the other files
        ...
        try {
            ...
            for(Resource serviceResource : sourceRestoreFolder.get("services").list()) {
                ...
                if (service != null && service.getWorkspace() == null) {
                    Files.delete(dd.get(serviceResource.name()).file());
                    Resources.copy(
                            sourceRestoreFolder.get(Paths.path("services", serviceResource.name())).file(), dd.get(Paths.BASE));
                }
            }
            ...
            // TODO: Workspace Local LayerGroups
            ...
            backupAdditionalResources(sourceGeoServerResourceLoader, dd.get(Paths.BASE));
            ...
            TODO:
                ...
                - I'm pretty sure to forgot something!!!
