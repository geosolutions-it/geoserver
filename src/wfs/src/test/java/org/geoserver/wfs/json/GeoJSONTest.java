...
public class GeoJSONTest extends WFSTestSupport {
    ...
    public void testFeatureBoundingDisabledCollection() throws Exception {
        /*
         * In GML we have the option not to compute the bounds in the response, and by default we don't, but GeoServer can be configured to return the
         * bounds, in that case it will issue a bounds query against the store, which might take a long time (that depends a lot on what the store can
         * do, some can compute it quickly, no idea what SDE). For GeoJSON it seems that the "feature bounding" flag is respected for the single
         * feature bounds, but not for the collection. Looking at the spec ( http://geojson.org/geojson-spec.html ) it seems to me the collection bbox
         * is not required: "To include information on the coordinate range for geometries, features, or feature collections, a GeoJSON object may
         * have a member named "bbox"" disable Feature bounding
         */
        ...
        try {
            ...
            String out = getAsString("wfs?request=GetFeature&version=1.0.0&typename=sf:AggregateGeoFeature&maxfeatures=3&outputformat="
                    + JSONType.json);
    ...
    public void testPost() throws Exception {
        String xml = "<wfs:GetFeature " + "service=\"WFS\" " + "outputFormat=\""+JSONType.json+"\" "
                + "version=\"1.0.0\" " + "xmlns:cdf=\"http://www.opengis.net/cite/data\" "
    ...
    public void testMixedCollection() throws Exception {
        String xml = "<wfs:GetFeature " + "service=\"WFS\" " + "outputFormat=\""+JSONType.json+"\" "
                ...
                + "<wfs:Query typeName=\"sf:AggregateGeoFeature\" /> " + "</wfs:GetFeature>";
        // System.out.println("\n" + xml + "\n");
        ...
        assertTrue(aFeature.getString("id").substring(0, 19)
                .equalsIgnoreCase("PrimitiveGeoFeature"));
        aFeature = featureCol.getJSONObject(6);
        // System.out.println(aFeature.getString("id").substring(0,19));
        assertTrue(aFeature.getString("id").substring(0, 19)
                .equalsIgnoreCase("AggregateGeoFeature"));
