...
public class AccessLimitsKeyBuilder implements InitializingBean {
    ...
    private ObjectNode buildKeyNode(AccessLimits limits) {
        ...
        if (limits instanceof VectorAccessLimits) return buildVectorNode((VectorAccessLimits) limits);
        if (limits instanceof CoverageAccessLimits) return buildCoverageNode((CoverageAccessLimits) limits);
        ...
        if (limits instanceof WMSAccessLimits) {
            WMSAccessLimits wms = (WMSAccessLimits) limits;
            return buildRasterDataNode(wms.getReadFilter(), wms.getRasterFilter());
        }
        if (limits instanceof WMTSAccessLimits) {
            WMTSAccessLimits wmts = (WMTSAccessLimits) limits;
            return buildRasterDataNode(wmts.getReadFilter(), wmts.getRasterFilter());
        }
        if (limits instanceof DataAccessLimits) return buildDataNode((DataAccessLimits) limits);
    ...
    private String serializeValue(String paramName, Object value) {
        ...
        if (value instanceof Filter) return serializeFilter((Filter) value);
        if (value instanceof Geometry) return serializeGeometry((Geometry) value);
        if (value instanceof Date) return DATE_FORMAT.format(((Date) value).toInstant());
    ...
    private static String computeCrsCode(Geometry geom) {
        ...
        if (userData instanceof CoordinateReferenceSystem) {
            try {
                ...
                String id = ResourcePool.lookupIdentifier((CoordinateReferenceSystem) userData, false);
    ...
    public String buildLayerGroupKey(List<String> layerNames, List<AccessLimits> limits) {
        ...
        for (int i = 0; i < layerNames.size(); i++) {
            ...
            if (layerNode != null && !layerNode.isEmpty()) {
                anyRestricted = true;
                for (Map.Entry<String, JsonNode> property : layerNode.properties()) {
                    entry.set(property.getKey(), property.getValue());
                }
            }
            array.add(entry);
        }
    ...
    private String buildAndLimit(ArrayNode array) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            if (node instanceof ObjectNode) entries.add((ObjectNode) node);
        }
    ...
    public class Field {
        ...
        public Field(ObjectNode node, String name, String value) {
            this.node = node;
            this.name = name;
            this.value = value;
        }
        ...
        public boolean equals(Object o) {
            ...
            Field field = (Field) o;
            return Objects.equals(node, field.node)
    ...
    private void truncateLongestFields(List<ObjectNode> nodes, Supplier<String> serialized) {
        ...
        for (ObjectNode n : nodes) {
            for (Map.Entry<String, JsonNode> nodeEntry : n.properties()) {
                String fname = nodeEntry.getKey();
                JsonNode v = nodeEntry.getValue();
                if (v.isTextual()) fields.add(new Field(n, fname, v.textValue()));
            }
        }
        // longest value first - each truncation removes the most bytes
        fields.sort(Comparator.comparingInt(f -> -f.getValue().length()));
        ...
        for (Field f : fields) {
            ...
            f.getNode().put(f.getName(), orig.substring(0, prefix) + TRUNCATION_SUFFIX + sha256hex(orig));
        }
    }
    ...
    private static String errorDuplicateSerializer(Class<?> valueType, String first, Class<?> second) {
        return String.format(
                "Duplicate ParameterValueKeySerializer for value type %s: %s and %s",
                valueType.getName(), first, second.getName());
    }

    private static String errorUnknownParam(String code, GeneralParameterValue gpv) {
        return String.format(
                "Cannot build security cache key for parameter '%s': GeneralParameterValue type %s is not a ParameterValue and has no contributed serializer. Contribute a ParameterValueKeySerializer bean or add '%s' to the %s system property if it does not affect tile content.",
                code, gpv.getClass().getName(), code, IgnorableParameterRegistry.SYSTEM_PROPERTY);
    }

    private static String errorUnknownValue(String paramName, Object value) {
        return String.format(
                "Cannot build security cache key for parameter '%s': no ParameterValueKeySerializer found for value type %s. Contribute a ParameterValueKeySerializer bean or add '%s' to the %s system property if it does not affect tile content.",
