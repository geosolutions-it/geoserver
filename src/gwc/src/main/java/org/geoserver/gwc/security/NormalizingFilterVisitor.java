...
class NormalizingFilterVisitor extends SimplifyingFilterVisitor {
    ...
    public Object visit(PropertyIsEqualTo filter, Object extraData) {
        ...
        if (result instanceof BinaryComparisonOperator) {
            BinaryComparisonOperator op = (BinaryComparisonOperator) result;
            if (isLiteralLeft(op))
        ...
        if (result instanceof BinaryComparisonOperator) {
            ...
            if (isLiteralLeft(op))
                ...
                return getFactory(extraData)
                        .greaterOrEqual(op.getExpression2(), op.getExpression1(), op.isMatchingCase());
        }
        return result;
    }
    ...
    static class Keyed {
        ...
        public Keyed(Filter filter, String type, String ecql) {
            this.filter = filter;
            this.type = type;
            this.ecql = ecql;
        }
    ...
    private static List<Filter> sorted(List<Filter> children) {
        ...
        return children.stream()
                ...
                .sorted(Comparator.comparing(Keyed::getType).thenComparing(Keyed::getEcql))
                .map(Keyed::getFilter)
