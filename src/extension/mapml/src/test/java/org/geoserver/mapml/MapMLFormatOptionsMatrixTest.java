...
public class MapMLFormatOptionsMatrixTest extends MapMLTestSupport {
    ...
    private void setupTestLayers() {
        try {
            createMixedLayerGroup(MIXED_GROUP_CACHED, false);
            createMixedLayerGroup(MIXED_GROUP_NOCACHE, false);
        } catch (Exception e) {
            throw new RuntimeException("Failed to set up test layers", e);
        }
    }

    private void createMixedLayerGroup(String name, boolean cached) throws Exception {
    ...
    public void testScenarioC2_WithTileCache() throws Exception {
        ...
        try {
            ...
            assertTrue("Tile URL should not contain mapmlfeatures:true", !tileUrl.contains("mapmlfeatures:true"));
            assertTrue(
