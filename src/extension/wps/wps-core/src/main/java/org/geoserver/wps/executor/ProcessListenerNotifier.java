...
public class ProcessListenerNotifier {
    ...
    public void fireProgress(float progress, String task) {
        if (progress > status.progress || StringUtils.equals(task, status.task)) {
            ...
            if (progress > 0) {
                long timeEalpsedMillis = (new Date().getTime() - status.getCreationTime().getTime());
                int estimatedCompletionMillis = (int) ((timeEalpsedMillis / progress) * timeEalpsedMillis + timeEalpsedMillis);
