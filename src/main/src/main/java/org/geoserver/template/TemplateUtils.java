...
public class TemplateUtils {
     ...
     * Get a Freemarker configuration that is safe against malicious templates
     *
     * @param wrapper the wrapper to be modified; a new {@link DefaultObjectWrapper} will be created
     *     if this is {@code null}
     * @param policy the access policy to be used; a default access policy will be assigned if this
     *     is {@code null} based on the specified {@code exposureLevel}
     * @param exposureLevel should be one of {@link BeansWrapper#EXPOSE_SAFE}, {@link
     *     BeansWrapper#EXPOSE_PROPERTIES_ONLY} or {@link BeansWrapper#EXPOSE_NOTHING}, {@code
     *     EXPOSE_SAFE} is the default if this is {@code null}
     */
    public static Configuration getSafeConfiguration(
        ...
        templateConfig.setNewBuiltinClassResolver(
                (name, env, template) -> {
     ...
     *     EXPOSE_SAFE} is the default if this is {@code null}
     */
    public static BeansWrapper getSafeWrapper(
            BeansWrapper wrapper, MemberAccessPolicy policy, Integer exposureLevel) {
        wrapper = wrapper != null ? wrapper : new DefaultObjectWrapper(FM_VERSION);
        int level = exposureLevel != null ? exposureLevel : EXPOSE_SAFE;
        wrapper.setMemberAccessPolicy(
                policy != null ? policy : (level <= EXPOSE_SAFE ? DEFAULT_ACCESS : LIMIT_ACCESS));
