...
/**
 * A custom member access policy implementation that provides stricter restrictions that FreeMarker
 * does by default for what fields and methods can be accessed and allows an administrator to set
 * system properties to control these restrictions. A field/method is only exposed if the class of
 * the template object and the class of the field or the method's return type are in the list of
 * allowed classes/packages and not in the list of blocked classes/packages.
 *
 * <p>An additional property controls whether to allow access to only getter methods or to all
 * methods. While the allow and block lists should be sufficient to ensure that templates are safe,
 * limiting templates to only getter methods will help prevent new vulnerabilities from being
 * introduced as new capabilities are added to GeoServer's API.
 */
public final class GeoServerMemberAccessPolicy implements MemberAccessPolicy {
    ...
    private static final DefaultMemberAccessPolicy DEFAULT_POLICY =
            DefaultMemberAccessPolicy.getInstance(FM_VERSION);

    /** Default list of classes and packages to allow */
    private static final List<Object> DEFAULT_ALLOW =
            Arrays.asList(
                    "java.",
    ...
    private static final List<Object> DEFAULT_BLOCK =
            Arrays.asList(
                    ...
                    GridCoverageWriter.class);

    /** Block all static field/method access by default */
    private static final Predicate<Class<?>> DEFAULT_STATIC_ACCESS = clazz -> false;

    /**
     * Policy that uses the system property to determine whether to allow access to all methods or
     * only getter methods that pass other safety checks
     */
    public static final GeoServerMemberAccessPolicy DEFAULT_ACCESS =
            new GeoServerMemberAccessPolicy(null, null, null);

    /**
     * Policy that that ignores the system property and always allow access to all methods that pass
     * other safety checks
     */
    public static final GeoServerMemberAccessPolicy FULL_ACCESS =
            new GeoServerMemberAccessPolicy(true, null, null);

    /**
     * Policy that that ignores the system property and always allow access to only getter methods
     * that pass other safety checks
     */
    public static final GeoServerMemberAccessPolicy LIMIT_ACCESS =
            new GeoServerMemberAccessPolicy(false, null, null);
    ...
    private GeoServerMemberAccessPolicy(
            Boolean apiExposed, List<Object> allowList, Predicate<Class<?>> staticAccess) {
        this.forceApiExposed = apiExposed;
        this.apiExposed = this.forceApiExposed;
        this.forceAllowList = allowList != null ? allowList : Collections.emptyList();
    ...
    public GeoServerMemberAccessPolicy withAllowList(Object... allowList) {
        return new GeoServerMemberAccessPolicy(
                this.forceApiExposed, Arrays.asList(allowList), this.staticAccess);
    }
    ...
    public GeoServerMemberAccessPolicy withStaticAccess(Predicate<Class<?>> staticAccess) {
        return new GeoServerMemberAccessPolicy(
                this.forceApiExposed, this.forceAllowList, staticAccess);
    }
    ...
    /**
     * Resets the fields so that they will be reloaded from the system properties the next time they
     * are needed. This is intended for unit tests only.
     */
    ...
    /**
     * Gets the list of classes and packages to allow, initializing it from the system property the
    ...
    /**
     * Gets the list of classes and packages to block, initializing it from the system property the
     * first time it is called
     */
    ...
    /**
     * Checks whether to allow full API access or restrict access to getter methods only,
     * initializing it from the system property the first time it is called.
     */
    ...
    private synchronized void init() {
            ...
            if (!this.forceAllowList.isEmpty()) {
                list =
                        Collections.unmodifiableList(
                                Stream.concat(list.stream(), this.forceAllowList.stream())
        ...
        if (this.apiExposed == null) {
            this.apiExposed =
                    Boolean.parseBoolean(GeoServerExtensions.getProperty(FREEMARKER_API_EXPOSED));
        }
    }

    /**
     * Looks up the value of the system property with the provided key and parses it as a comma
     * separated list, check if each string is a valid class name or otherwise assumes that it is a
     * package name prefix, and appends the parsed values to the list of default values
     */
    private static List<Object> parseList(String key, List<Object> defaults) {
        ...
        return Collections.unmodifiableList(
                Stream.concat(defaults.stream(), stream).collect(Collectors.toList()));
    }
    ...
    private final class GeoServerClassMemberAccessPolicy implements ClassMemberAccessPolicy {
        ...
        public boolean isMethodExposed(Method method) {
            if (!this.defaultPolicy.isMethodExposed(method)) {
                ...
                // allow toString() for backwards-compatibility even though templates shouldn't call
                // it directly
                return true;
            }
        ...
        /**
         * Check if the system property restricts access to only getter methods and if the
         * restriction is enabled, check if the method is a zero-argument method with a name that
         * equals "toString", a name that starts with "get" and has a non-void return type or a name
         * the starts with "is" and has a boolean return type.
         */
        ...
        /**
         * Checks if the provided class is either a proxy of a GeoServer interface, a primitive type
         * or it is in the allow list and not in the block list
         */
        private boolean isClassAllowed(Class<?> clazz) {
            if (clazz.equals(void.class)) {
                ...
                return !interfaces.isEmpty()
                        && interfaces.stream().noneMatch(c -> matchesAny(c, getBlockList()));
            }
            ...
            return actual.isPrimitive()
                    || (matchesAny(actual, getAllowList()) && !matchesAny(actual, getBlockList()));
        }
