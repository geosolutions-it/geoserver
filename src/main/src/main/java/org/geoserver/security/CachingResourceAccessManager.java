...
public class CachingResourceAccessManager extends ResourceAccessManagerWrapper {
    ...
    private final class Key {
        ...
        private Key(Kind kind, String user, String target, String containers) {
            this.kind = kind;
            this.user = user;
            this.target = target;
        ...
        @Override
        public int hashCode() {
            return Objects.hash(kind, user, target, containers);
        }
    ...
    private <T> T cached(Kind kind, Authentication auth, String targetId, String containers, Supplier<T> loader) {
        ...
        // bypass if there is no current request context (e.g., in a computing background thread)
        RequestAttributes attrs = RequestContextHolder.getRequestAttributes();
        if (attrs == null) return loader.get();

        Map<Key, Object> cache = (Map<Key, Object>) attrs.getAttribute(CACHE_ATTR, RequestAttributes.SCOPE_REQUEST);
        if (cache == null) {
            cache = new HashMap<>();
